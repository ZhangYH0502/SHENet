...
class UNet(nn.Module):
    def __init__(self, n_in=7, n_out=1, first_channels=64, n_dps=4, use_bilinear=False, use_pool=False,
                 norm_type='instance', device='cpu'):
        ...
        self.gcn = GcnPred(nodes_num=12*12, feats_dim=1024, device=device)
    ...
    def forward(self, x):
        ...
        f0 = self.projhead(features[-1])
        enc_fea.append(f0)

        features[-1] = self.gcn(features[-1])

        f1 = self.projhead(features[-1])
        enc_fea.append(f1)
...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, use_pool, norm_layer):
        ...
        if use_pool:
            ...
            self.down_conv = nn.Sequential(
                norm_layer(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=2, padding=1)),
                nn.ReLU(inplace=True),
                norm_layer(nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)),
                nn.ReLU(inplace=True),
                SeBlock(out_channels, out_channels)
...
class GcnPred(nn.Module):
    def __init__(self, nodes_num=12*12, feats_dim=1024, device='cpu'):
        ...
        self.uPred.append(GATConv(self.feats_dim, self.n_hidden, num_heads=self.num_heads, residual=True, activation=F.relu))
        self.uPred.append(GATConv(self.n_hidden, self.n_hidden, num_heads=self.num_heads, residual=True, activation=F.relu))
        self.uPred.append(GATConv(self.n_hidden, self.feats_dim, num_heads=self.num_heads, residual=True, activation=None))
